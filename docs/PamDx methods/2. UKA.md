...
1. **Assigning confidence values for Kinase-Substrate Relations**
...
![legacy UKA matrix](Attachments/uka_07.png)
...
$\mathbf{s}$: the Kinase Statistic is the weighted average of $\mathbf{p}$, the peptide difference statistic. 
$\mathbf{b}$ is a normalization vector,
$\mathbf{a}_{\mathbf{i}}$ is the number of nonzero elements in column $i$ of $M$ (the number of peptides that a kinase phosphorylates).
...
$$\mathbf{s}_{\mathbf{MKC}}\mathbf{= b\ *}\left( \mathbf{M}^{\mathbf{T}}\mathbf{p}_{\mathbf{change}} \right)$$
...
For legacy UKA, mean and median scores are calculated across the different peptide sets (different M matrices) considered for a kinase. For further analysis, the Median Kinase Statistic and Median Final Score are used.